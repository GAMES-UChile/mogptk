...
class Prediction:
    ...
    def predict(self, model):
        self.Y_mu_pred, self.Y_var_pred = model.predict(self.X_pred)
    ...
    def plot(self, filename=None, title=None):
        ...
        for channel in channels:
            if channel in self.Y_mu_pred:
                lower = self.Y_mu_pred[channel] - self.Y_var_pred[channel]
                upper = self.Y_mu_pred[channel] + self.Y_var_pred[channel]

                for i in range(data.get_input_dims()):
                    axes[channel, i].plot(self.X_pred[channel][:,i], self.Y_mu_pred[channel], 'b-', lw=3)
                    axes[channel, i].fill_between(self.X_pred[channel][:,i], lower, upper, color='b', alpha=0.1)
                    axes[channel, i].plot(self.X_pred[channel][:,i], lower, 'b-', lw=1, alpha=0.5)
                    axes[channel, i].plot(self.X_pred[channel][:,i], upper, 'b-', lw=1, alpha=0.5)
                plotting_pred = True

                if channel in data.F:
                    for i in range(data.get_input_dims()):
                        n = (len(data.X[channel][:,i]) + len(self.X_pred[channel][:,i]))*10
                        x_min = np.min(np.concatenate((data.X[channel][:,i], self.X_pred[channel][:,i])))
                        x_max = np.max(np.concatenate((data.X[channel][:,i], self.X_pred[channel][:,i])))
