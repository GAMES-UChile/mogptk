...
class MemTracker(object):
    ...
    def __init__(self, detail=True, path='', verbose=False, device=0):
        ...
        self.gpu_profile_fn = path + f'memory.txt'
    ...
    def track(self, back=0):
        """
        Track the GPU memory usage
        """
        frameinfo = inspect.stack()[1+back]
        ...
        with open(self.gpu_profile_fn, 'a') as f:
            ...
            f.write(f"\n{where_str:<50}"
                    f" tensors={self.get_tensor_usage():.1f} MB"
                    f" total={self.get_allocate_usage():.1f} MB\n")
            ...
            if self.print_detail is True:
                ...
                for t, s, n, m, data_type in new_tensor_sizes - self.last_tensor_sizes:
                    f.write(f'+ | {str(n):5} * Size:{str(s):<20} | Memory: {m*n:5.1f} MB | {data_type}\n')
